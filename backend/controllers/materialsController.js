const service = require("../services/materialsService");

// ===== GET =====
exports.getAll = async (req, res) => {
  try {
    const data = await service.getMaterials();
    res.json(data);
  } catch (err) {
    console.error("GET MATERIALS ERROR:", err.message);
    res.status(500).json({ message: err.message });
  }
};

// ===== CREATE =====
exports.create = async (req, res) => {
  try {
    const user = req.user;

    const item = await service.createMaterial(req.body, user);

    res.status(201).json(item);
  } catch (err) {
    console.error("CREATE MATERIAL ERROR:", err.message);
    res.status(400).json({ message: err.message });
  }
};

// ===== UPDATE =====
exports.update = async (req, res) => {
  try {
    const user = req.user;

    const item = await service.updateMaterial(req.params.id, req.body, user);

    res.json(item);
  } catch (err) {
    console.error("UPDATE MATERIAL ERROR:", err.message);
    res.status(400).json({ message: err.message });
  }
};

// ===== DELETE =====
exports.remove = async (req, res) => {
  try {
    const user = req.user;

    await service.deleteMaterial(req.params.id, user);

    res.json({ message: "Deleted successfully" });
  } catch (err) {
    console.error("DELETE MATERIAL ERROR:", err.message);
    res.status(400).json({ message: err.message });
  }
};