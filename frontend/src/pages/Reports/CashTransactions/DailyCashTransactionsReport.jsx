...
export default function DailyCashTransactionsReport() {
  ...
  const filteredTransactions = transactions.filter(
    (item) => item.transaction_date === selectedDate,
  );
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save("daily-cash-transactions-report.pdf");
  };
      ...
      <div id="report-root" className="flex flex-col items-center w-full">
        {sortedTransactions.length === 0 ? (
          <div className="pdf-page w-full max-w-[210mm] mx-auto bg-white p-4">
            <ReportHeader title={`Daily Transactions - ${selectedDate}`} />
              ...
              <table className="w-full text-xs border border-gray-300 border-collapse mt-3 table-fixed">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="border p-2 w-[25%]">Description</th>
                  </tr>
                </thead>
                ...
                {pageIndex === pages.length - 1 && (
                  <tfoot className="bg-gray-100 font-bold">
                    <tr>
                      ...
                      <td colSpan="1" className="border p-2 text-red-600">
                      ...
                      <td
                        colSpan="1"
                        className={`border p-2 ${profit >= 0 ? "text-green-700" : "text-red-700"}`}
                      >
                        {profit.toFixed(2)}
                      </td>
                    </tr>
