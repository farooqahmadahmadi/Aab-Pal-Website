...
export default function Suppliers() {
    ...
    useEffect(() => {
        const f = data.filter(i =>
            ...
            i.supplier_phone?.toLowerCase().includes(search.toLowerCase()) ||
            i.supplier_email?.toLowerCase().includes(search.toLowerCase()) ||
            i.supplier_status?.toLowerCase().includes(search.toLowerCase()) ||
            i.supplier_address?.toLowerCase().includes(search.toLowerCase())
        );
        setFiltered(f);
        setPage(1);
    }, [search, data]);
                    ...
                    <tbody>
                        {paginated.length ? paginated.map(i => (
                            <tr key={i.supplier_id} className="border-t hover:bg-gray-50">
                                <td>{i.supplier_id}</td>
                                <td className="text-left">{i.supplier_name}</td>
                                <td>{i.supplier_phone}</td>
                                <td className="text-left"><a href="mailto:">{i.supplier_email}</a></td>
                                <td className="text-left">{i.supplier_address}</td>
                                <td >
                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold
                                        ${i.supplier_status === "Active" ? "bg-green-100 text-green-600" : "bg-gray-200 text-gray-600"}`}>
                                        {i.supplier_status}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colSpan="7" className="p-4 text-gray-500">No suppliers records found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {deleteData && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-6 rounded w-96">
                        <p>Delete <strong>{deleteData.supplier_name}</strong>?</p>
